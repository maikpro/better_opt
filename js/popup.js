...
async function displayCurrentFolge(){
    ...
    $("#currentFolgeLink").attr("href", "https://onepiece-tube.com/folge/" + currentFolge);
    $("#currentFolge").text(currentFolge);

    //previous
    $("#previousFolgeLink").attr("href", "https://onepiece-tube.com/folge/" + (parseInt(currentFolge, 10) - 1));

    //next
    $("#nextFolgeLink").attr("href", "https://onepiece-tube.com/folge/" + (parseInt(currentFolge, 10) + 1));
}

async function displayFolgenList(){
    const folgenList = await OPTStorageManager.getLocal("folgenList");

    let options = folgenList.map((folge) => {
